...
export const parseBeatmap: (
  ...
  const isBeatsage = infoJSON?._customData?._editors?.beatsage !== undefined
  const isLegacyBeatsage = infoJSON?._customData?.beatSage !== undefined
  const hasBeatSageAuthor = infoJSON._levelAuthorName === 'Beat Sage'
  const automapper =
    isBeatsage || isLegacyBeatsage || hasBeatSageAuthor ? 'beatsage' : null
